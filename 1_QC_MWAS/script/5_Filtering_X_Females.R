...
print("5 - FILTERING X CHR IN FEMALES")
...
outlier <- c() #identify the outlier at the end of the script
...
################################################################################
# GET CPGS ON X & SAMPLE NAMES OF FEMALES
################################################################################

# EXTRACT CPGs ON SEX CHROMOSOMES FROM ANNOTATION FILE
if (array.type == "EPICv2"){
  cgs <- annotation[substr(annotation$Name, start = 1, stop = 2) == "cg" & (annotation$CHR == "chrX"),]
}

if (array.type == "EPICv1" | array.type == "450K"){
  cgs <- annotation[substr(annotation$Name, start = 1, stop = 2) == "cg" & (annotation$CHR == "X"),]
}
...
# SAMPLE NAMES
samples <- intersect(colnames(TypeI.Red.M), samplesheet$Basename[which(samplesheet$Sex == "F")])
...
print("Step 2/5: filter intensities on X chr CpGs and females.")
...
samplesheet$to_exclude1e16_XX <- NA
samplesheet$to_exclude1e16_XX[which(samplesheet$Basename %in% samples_to_excl)] <- "exclude"
...
save(CpGs_low_call_rate, file = "output/RData/Low_marker_call_rate_1e16_X_females.RData")
...
save(samples_low_call_rate, file = "output/RData/Low_sample_call_rate_1e16_X_females.RData")
low_sample_call_df <- data.frame (sample = samples_low_call_rate, call_rate_1e16 = low_sample_call_rate)
save(sample.call, marker.call, file='output/RData/callRates_1e16_X_females.RData')
...
samplesheet$to_exclude1e16_XX[which(samplesheet$Basename %in% low_sample_call_df$sample)] <- "exclude"
samplesheet$to_exclude1e16_XX[which(samplesheet$Basename %in% outlier)] <- "exclude"
...
save(TypeII.Red ,TypeII.Green ,TypeI.Red.M ,TypeI.Red.U ,TypeI.Green.M ,TypeI.Green.U , 
     file="output/RData/intensities_filtered_1e16_X_females.RData",compress=F)
...
pdf("output/Figures/Distributions/Beta_distribution_filtered_1e16_X_females.pdf", width = 7, height = 5)
densityPlot(beta, main = "Filtered beta values")
dev.off()

# VISUALISE FILTERED M VALUE DISTRIBUTION
pdf("output/Figures/Distributions/Mvalue_distribution_filtered_1e16_X_females.pdf", width = 7, height = 5)
...
suppressWarnings({
  ...
  ggsave("output/Figures/Distributions/Filtered_beta_outlier_check_X_females.pdf", plot = dens_plot_lowintensity, device = "pdf", width=10, height=6, dpi=400)
})
