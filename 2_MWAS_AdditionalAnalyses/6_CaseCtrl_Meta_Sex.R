...
beta_path <- "S://Project/WP-epigenetics//04_Pipeline_PhaseI/output/RData/beta_final_combined_males_females.RData"
Mvalues_path <- "S://Project/WP-epigenetics/04_Pipeline_PhaseI/output/RData/Mvalues_final_combined_males_females.RData"
...
samplesheet_path <- "S://Project/WP-epigenetics/04_Pipeline_PhaseI/output/Tables/samplesheet_after_QC.xlsx"
...
# LOAD EWAS RESULTS
load("output_F/RData/DMPs_CaseCtrl_15CTRLPCs_2AncPCs_limma_anno_BACON.RData")
Female_results <- DMPs_anno
load("output_M/RData/DMPs_CaseCtrl_5CTRLPCs_2AncPCs_limma_anno_BACON.RData")
Male_results <- DMPs_anno
rm(DMPs_anno)
...
samples <- samplesheet[which(is.na(samplesheet$Transgender) & samplesheet$Baseline_sample == "Y"), ]
...
# MAKE FOLDERS
if (!dir.exists("output_meta/")) dir.create("output_meta/")
if (!dir.exists("output_meta/Figures")) dir.create("output_meta/Figures")
if (!dir.exists("output_meta/RData")) dir.create("output_meta/RData")
if (!dir.exists("output_meta/Tables")) dir.create("output_meta/Tables")

if (!dir.exists("output_meta/Figures/DMRs")) dir.create("output_meta/Figures/DMRs")
if (!dir.exists("output_meta/Figures/DMRs/raw")) dir.create("output_meta/Figures/DMRs/raw")
if (!dir.exists("output_meta/Figures/DMRs/combined")) dir.create("output_meta/Figures/DMRs/combined")

if (!dir.exists("output_meta/Figures/DMPs")) dir.create("output_meta/Figures/DMPs")
if (!dir.exists("output_meta/Figures/DMPs/raw")) dir.create("output_meta/Figures/DMPs/raw")
if (!dir.exists("output_meta/Figures/DMPs/combined")) dir.create("output_meta/Figures/DMPs/combined")

if (!dir.exists("output_meta/Figures/mQTL")) dir.create("output_meta/Figures/mQTL")
if (!dir.exists("output_meta/Figures/mQTL/combined")) dir.create("output_meta/Figures/mQTL/combined")
if (!dir.exists("output_meta/Figures/mQTL_DNAm")) dir.create("output_meta/Figures/mQTL_DNAm")

################################################################################
# META ANALYSIS
################################################################################

# IDENTIFY PROBES IN BOTH Meta AND Meta
probes <- intersect(unique(Female_results$cgID), unique(Male_results$cgID))

# COMBINE SUM STATS
Female_results_red <- Female_results[which(Female_results$cgID %in% probes), c("cgID", "effectsize", "SE", "P.Value")]
colnames(Female_results_red) <- c("cgID", "effectsize_Female", "SE_Female", "P.Value_Female")
Male_results_red <- Male_results[which(Male_results$cgID %in% probes), c("cgID", "effectsize", "SE", "P.Value")]
colnames(Male_results_red) <- c("cgID", "effectsize_Male", "SE_Male", "P.Value_Male")
merged_sumstats <- merge(Female_results_red, Male_results_red, by = "cgID")
rm(Female_results_red, Male_results_red)

# RUN META-ANALYSIS

meta_function <- function(x){
  
  study_values <- data.frame(
    study = c("Female", "Male"),
    effect_size = as.numeric(c(x[2], x[5])),
    SE = as.numeric(c(x[3], x[6]))
  )
  ...
  cgID <- x[1]
  meta_effect <- meta_analysis$TE.random[1]
  meta_SE <- meta_analysis$seTE.random[1]
  meta_p <- meta_analysis$pval.random[1]
  tau <- meta_analysis$tau #sd of the effects across studies in random-effects meta-analysis 
  i2 <- meta_analysis$I2 #% of variation across studies that is due to heterogeneity rather than chance
  q <- meta_analysis$Q # tests heterogeneity among studies in the meta-analysis
  q_p <- meta_analysis$pval.Q # p value associated with Q statistic
  effectsize_F <- x[2]
  SE_F <- x[3]
  P_F <- x[4]
  effectsize_M <- x[5]
  ...
  combined_result <- c(cgID, meta_effect, meta_SE, meta_p, tau, i2, q, q_p, effectsize_F, SE_F, P_F, effectsize_M, SE_M, P_M)
  
  return(combined_result)
}


sum_stats_list <- as.list(data.frame(t(merged_sumstats)))
#sum_stats_list_red <- sum_stats_list[1:5] #was used to test

tic()
results_meta_list <- lapply(sum_stats_list, meta_function)
results_meta <- as.data.frame(t(data.frame(results_meta_list)))
colnames(results_meta) <- c("cgID","Effect_Random", "SE_Random","P_Random", 
                            "tau", "I2", "Q", "Het_P",
                            "Effect_Female", "SE_Female", "P_Female", "Effect_Male", "SE_Male", "P_Male")
toc() #29343.15 sec elapsed - 8.150875h

results_meta$Effect_Random <- as.numeric(results_meta$Effect_Random)
results_meta$SE_Random <- as.numeric(results_meta$SE_Random)
results_meta$P_Random <- as.numeric(results_meta$P_Random)
results_meta$tau <- as.numeric(results_meta$tau)
results_meta$I2 <- as.numeric(results_meta$I2)
results_meta$Q <- as.numeric(results_meta$Q)
results_meta$Het_P <- as.numeric(results_meta$Het_P)
results_meta$Effect_Female <- as.numeric(results_meta$Effect_Female)
results_meta$SE_Female <- as.numeric(results_meta$SE_Female)
results_meta$P_Female <- as.numeric(results_meta$P_Female)
results_meta$Effect_Male <- as.numeric(results_meta$Effect_Male)
results_meta$SE_Male <- as.numeric(results_meta$SE_Male)
results_meta$P_Male <- as.numeric(results_meta$P_Male)

DMPs <- results_meta

save(DMPs, file = "output_meta/RData/DMPs_CaseCtrl_meta.RData")

# ADJUST DMPs
colnames(DMPs) <- c("cgID", "Effect", "SE", "P.Value", "tau", "I2", "Q", "Het_P", 
                    "Effect_Female", "SE_Female", "P_Female", "Effect_Male", "SE_Male", "P_Male")

# ADJUST FOR MULTIPLE TESTING AND EXPORT
DMPs$adj_p_BH <- p.adjust(DMPs$P.Value, method = "BH")

save(DMPs, file = "output_meta/RData/DMPs_meta_CaseCtrl_adapted.RData")
DMPs_anno <- merge(DMPs, anno_upd, by.x = "cgID", by.y = "Name", all.x = TRUE)
save(DMPs_anno, file = "output_meta/RData/DMPs_meta_CaseCtrl_anno.RData")


# EXPORT SIGNIFICANT RESULTS
signif_DMPs_BH <- DMPs_anno[which(DMPs_anno$adj_p_BH < 0.05),]
length(unique(signif_DMPs_BH$cgID)) #35
write_xlsx(signif_DMPs_BH, path = "output_meta/Tables/DMPs_meta_CaseCtrl_signif_BH.xlsx")

pdf(file = "output_meta/Figures/QQplot_meta_CaseCtrl.pdf", width = 5, height = 5)
qq(DMPs$P.Value)
dev.off()

png(file = "output_meta/Figures/QQplot_meta_CaseCtrl.png", width = 5, height = 5, units = "in", res = 300)
qq(DMPs$P.Value)
dev.off()

lambda(DMPs$P.Value) #0.9233878


png(file = "output_meta/Figures/QQplot_meta_CaseCtrl_heterogeneity.png", width = 5, height = 5, units = "in", res = 300)
qq(DMPs$Het_P)
dev.off()

lambda(DMPs$Het_P) #0.9327314
...
combp_result <- as.data.frame(read_xlsx("output_meta/Tables/combp_CaseCtrl_meta.xlsx"))


combp_sign <- adapt_combp(combp_result, DMPs_anno, anno_upd, nr_CpGs = 3, nr_signif_CpGs = 3, signif_proportion = 0.5)
#"significant DMRs before filtering step: 34"
#"significant DMRs after filtering: 17"

write_xlsx(combp_sign, "output_meta/Tables/DMRs_combp_CaseCtrl_filtered_BACON.xlsx")
...
ggsave("output_meta/Figures/Manhattan_CaseCtrl.png", Manhattan_plot, units = "cm", width = 13, height = 9, dpi = 400)
...
# SELECT M VALUES
# LOAD AND FILTER  VALUES
load(Mvalues_path)
Mvalues <- Mvalues[row.names(Mvalues) != "cg05575921", order(colnames(Mvalues))] # remove cpg that is used to adjust for smoking
...
# adjust beta values (not incl. ctrl_pc4 as high VIF with sex)
...
Mvalues_wobatch <- removeBatchEffect(Mvalues_filt, covariates = covar_matrix, batch = as.factor(samples$Sex))
rm(covar_matrix)
...
for (j in c(1:nrow(combp_sign))){
  ...
  ggsave(paste0("output_meta/Figures/DMRs/DMR_CaseCtrl_", GeneString2, "_DENS.svg"), Dens_plot, units = "cm", width = 13, height = 12, dpi = 400)
  ggsave(paste0("output_meta/Figures/DMRs/DMR_CaseCtrl_", GeneString2, "_DENS.pdf"), Dens_plot, units = "cm", width = 13, height = 12, dpi = 400) 
  save(Dens_plot, max_value, OR_bottom20, OR_top20,bottom20_thresh, top20_thresh, DMR_categ,   
       file = paste0("output_meta/Figures/DMRs/raw/DMR_CaseCtrl_", GeneString2, "_DENS.RData"))
  ...
  ggsave(paste0("output_meta/Figures/DMRs/DMR_CaseCtrl_", GeneString2, "_CATEG.pdf"), Categ_plot, units = "cm", width = 7, height = 10.5, dpi = 400)
  ggsave(paste0("output_meta/Figures/DMRs/DMR_CaseCtrl_", GeneString2, "_CATEG.svg"), Categ_plot, units = "cm", width = 7, height = 10.5, dpi = 400)
  save(Categ_plot, df_summary, GeneString, file = paste0("output_meta/Figures/DMRs/raw/DMR_CaseCtrl_", GeneString2, "_CATEG.RData"))
  ...
  combp_plot = combp.plot(region.chr = combp_sign$chr[j],
                          ...
                          estimate = DMPs_anno$Effect,
                          se = DMPs_anno$SE, 
                          chr = DMPs_anno$CHR,
                          pos = DMPs_anno$MAPINFO)
  
  ggsave(paste0("output_meta/Figures/DMRs/DMR_CaseCtrl_", GeneString2, "_combp.pdf"), combp_plot, units = "cm", width = 14, height = 10, dpi = 400)
  ggsave(paste0("output_meta/Figures/DMRs/DMR_CaseCtrl_", GeneString2, "_combp.svg"), combp_plot, units = "cm", width = 14, height = 10, dpi = 400)
  save(combp_plot, file = paste0("output_meta/Figures/DMRs/raw/DMR_CaseCtrl_", GeneString2, "_combp.RData"))
  ...
  ggsave(paste0("output_meta/Figures/DMRs/DMR_CaseCtrl_", GeneString2, "_CPG.svg"), cpg_plot, units = "cm", width = 17, height = 13.5, dpi = 400)
  ggsave(paste0("output_meta/Figures/DMRs/DMR_CaseCtrl_", GeneString2, "_CPG.pdf"), cpg_plot, units = "cm", width = 17, height = 13.5, dpi = 400)       
  save(cpg_plot, file = paste0("output_meta/Figures/DMRs/raw/DMR_CaseCtrl_", GeneString2, "_CPG.RData"))
  ...
  ggsave(paste0("output_meta/Figures/DMRs/DMR_CaseCtrl_", GeneString2, "_TRACK.svg"), cpg_line_plot, units = "cm", width = 20, height = 13.5, dpi = 400)
  ggsave(paste0("output_meta/Figures/DMRs/DMR_CaseCtrl_", GeneString2, "_TRACK.pdf"), cpg_line_plot, units = "cm", width = 20, height = 13.5, dpi = 400)       
  save(cpg_line_plot, GeneString, file = paste0("output_meta/Figures/DMRs/raw/DMR_CaseCtrl_", GeneString2, "_TRACK.RData"))
  ...
  ggsave(paste0("output_meta/Figures/DMRs/DMR_CaseCtrl_", GeneString2, "_TRACK_DIFF.svg"), cpg_line_plot_diff, units = "cm", width = 20, height = 10, dpi = 400)
  ggsave(paste0("output_meta/Figures/DMRs/DMR_CaseCtrl_", GeneString2, "_TRACK_DIFF.pdf"), cpg_line_plot_diff, units = "cm", width = 20, height = 10, dpi = 400)       
  save(cpg_line_plot_diff, GeneString, file = paste0("output_meta/Figures/DMRs/raw/DMR_CaseCtrl_", GeneString2, "_TRACK_DIFF.RData"))
  ...
  ggsave(paste0("output_meta/Figures/DMRs/combined/DMR_CaseCtrl_", GeneString2, "_COMB.svg"), layout, units = "cm", width = 22, height = 24, dpi = 400)
  ggsave(paste0("output_meta/Figures/DMRs/combined/DMR_CaseCtrl_", GeneString2, "_COMB.pdf"), layout, units = "cm", width = 22, height = 24, dpi = 400)       
...
write_xlsx(combp_sign, path = "output_meta/Tables/DMRs_CaseCtrl_extended.xlsx")
save(combp_sign, file = "output_meta/RData/DMRs_CaseCtrl_extended.RData")
...
signif_DMPs_BH$avg_OCD <- NA
signif_DMPs_BH$se_OCD <- NA
signif_DMPs_BH$median_OCD <- NA
signif_DMPs_BH$IQR_OCD <- NA

signif_DMPs_BH$avg_CTRL <- NA
signif_DMPs_BH$se_CTRL <- NA
signif_DMPs_BH$median_CTRL <- NA
signif_DMPs_BH$IQR_CTRL <- NA

signif_DMPs_BH$top20th <- NA
signif_DMPs_BH$bot20th <- NA
signif_DMPs_BH$top20_OR <- NA
signif_DMPs_BH$bottom20_OR <- NA
signif_DMPs_BH$top20_OR_CI <- NA
signif_DMPs_BH$bottom20_OR_CI <- NA
signif_DMPs_BH$top20_OR_pvalue <- NA
signif_DMPs_BH$bottom20_OR_pvalue <- NA



for (j in c(1:nrow(signif_DMPs_BH))){
  
  cgID <- signif_DMPs_BH$cgID[j]
  Gene <- signif_DMPs_BH$Gene[j]
  if (class(Gene) == "character" & Gene == ""){
    Gene <- "UNANNOTATED"
  }
  if (is.na(Gene)){
    Gene <- "UNANNOTATED"
  ...
  Gene2 <- gsub(";", "_", Gene)
  beta_cpg <- t(as.data.frame(beta_wobatch[rownames(beta_wobatch) == cgID, ]))
  rownames(beta_cpg) <- cgID
  
  
  # MAKE DF WITH CLINICAL INFO & AVERAGE DMR METHYLATION PER PATIENT
  CpG_categ <- samples[, c("Indiv_ID", "Diagnosis")]
  CpG_categ$DMPmethylation <- NA
  for(i in c(1:nrow(CpG_categ))){
    Indiv_name <- samples[i, "Indiv_ID"]
    sample_names <- samples$Basename[which(samples[, "Indiv_ID"] == Indiv_name)]
    CpG_categ$DMPmethylation[i] <- beta_cpg[, sample_names]
  }
  
  ####################
  
  # CALCULATE ORs and RRs
  CpG_categ$Diagnosis <- as.vector(CpG_categ$Diagnosis)
  CpG_categ$Diagnosis <- factor(CpG_categ$Diagnosis, levels = c("CTRL", "OCD"))
  
  # CALCULATE QUANTILES
  top20_thresh <- quantile(CpG_categ$DMPmethylation, 0.8)
  bottom20_thresh <- quantile(CpG_categ$DMPmethylation, 0.2)
  signif_DMPs_BH$top20th[j] <- top20_thresh
  signif_DMPs_BH$bot20th[j] <- bottom20_thresh
  
  # HIGHLIGHT INDIVIDUALS IN TOP/BOTTOM QUANTILES
  CpG_categ$top20 <- CpG_categ$DMPmethylation >= top20_thresh
  CpG_categ$bottom20 <- CpG_categ$DMPmethylation <= bottom20_thresh
  
  # CALCULATE ORs
  table_top20 <- table(CpG_categ$Diagnosis, CpG_categ$top20)
  OR_top20 <- fisher.test(table_top20)
  
  table_bottom20 <- table(CpG_categ$Diagnosis, CpG_categ$bottom20)
  OR_bottom20 <- fisher.test(table_bottom20)
  
  signif_DMPs_BH$top20_OR[j] <- OR_top20$estimate[1]
  signif_DMPs_BH$bottom20_OR[j] <- OR_bottom20$estimate[1]
  signif_DMPs_BH$top20_OR_CI[j] <- paste(round(OR_top20$conf.int[1:2],3), collapse = "-")
  signif_DMPs_BH$bottom20_OR_CI[j] <- paste(round(OR_bottom20$conf.int[1:2],3), collapse = "-")
  signif_DMPs_BH$top20_OR_pvalue[j] <- OR_top20$p.value[1]
  signif_DMPs_BH$bottom20_OR_pvalue[j] <- OR_bottom20$p.value[1]
  ...
  # CALCULATE HIGHEST DENSITY VALUE
  dens_value <- density(CpG_categ$DMPmethylation[which(CpG_categ$Diagnosis == "CTRL")])
  dens_value2 <- density(CpG_categ$DMPmethylation[which(CpG_categ$Diagnosis == "OCD")])
  max_value <- max(c(dens_value$y, dens_value2$y))
  
  # PLOT  DENSITY PLOTS WITH ADDED ORs
  Dens_plot <- ggplot(CpG_categ, aes(x = DMPmethylation, fill = Diagnosis)) + 
    geom_density(alpha = 0.5) + 
    scale_fill_manual(values = c("CTRL" = "#084081", "OCD" = "#891a1a")) +
    th + th_transparent +
    labs(x = "CpG DNAm", 
         y = "Density") +
    ggtitle(paste0(cgID, " - ", Gene)) +
    scale_y_continuous(expand=c(0,0), limits = c(0,max_value*1.3)) +
    scale_x_continuous(expand=c(0,0), limits = c(min(CpG_categ$DMPmethylation),max(CpG_categ$DMPmethylation))) +
    theme(legend.position = "top", legend.title = element_blank(), legend.box.background = element_blank(), plot.title = element_text(hjust = 0.5)) +
    geom_segment(aes(x=top20_thresh, y = 0, xend=top20_thresh, yend = max_value), colour = "black") +
    annotate("text", x = top20_thresh, y=max_value, label = paste0("top 20%\nOR: ", round(OR_top20$estimate[1], 1), sign_top20), vjust = -0.25) +
    geom_segment(aes(x=bottom20_thresh, y = 0, xend=bottom20_thresh, yend = max_value), colour = "black") +
  ...
  ggsave(paste0("output_meta/Figures/DMPs//DMP_CaseCtrl_", Gene2, "_DENS.svg"), Dens_plot, units = "cm", width = 13, height = 12, dpi = 400)
  ggsave(paste0("output_meta/Figures/DMPs/DMP_CaseCtrl_", Gene2, "_DENS.pdf"), Dens_plot, units = "cm", width = 13, height = 12, dpi = 400) 
  save(Dens_plot, max_value, OR_bottom20, OR_top20,bottom20_thresh, top20_thresh, CpG_categ, cgID, Gene,  
       file = paste0("output_meta/Figures/DMPs/raw/DMP_CaseCtrl_", cgID, "_", Gene2, "_DENS.RData"))
  
  # CALCULATE THE MEANS AND STANDARD ERRORS FOR EACH TIME POINT
  df_summary <- as.data.frame(CpG_categ %>%
                                group_by(Diagnosis) %>%
                                summarize(Mean = mean(DMPmethylation, na.rm = TRUE),
                                          se = sd(DMPmethylation, na.rm = TRUE) / sqrt(n()),
                                          median = median(DMPmethylation, na.rm = TRUE),
                                          IQR = IQR(DMPmethylation, na.rm = TRUE)))
  rownames(df_summary) <- levels(CpG_categ$Diagnosis)
  
  # ADD TO OVERVIEW DF
  signif_DMPs_BH$avg_OCD[j] <- round(df_summary["OCD","Mean"], 4)
  signif_DMPs_BH$avg_CTRL[j] <- round(df_summary["CTRL","Mean"], 4)
  
  signif_DMPs_BH$se_OCD[j] <- round(df_summary["OCD","se"], 4)
  signif_DMPs_BH$se_CTRL[j] <- round(df_summary["CTRL","se"], 4)
  
  signif_DMPs_BH$median_OCD[j] <- round(df_summary["OCD","median"], 4)
  signif_DMPs_BH$median_CTRL[j] <- round(df_summary["CTRL","median"], 4)
  
  signif_DMPs_BH$IQR_OCD[j] <- round(df_summary["OCD","IQR"], 4)
  signif_DMPs_BH$IQR_CTRL[j] <- round(df_summary["CTRL","IQR"], 4)
  
  
  # CATEGORY PLOT
  Categ_plot <- ggplot(CpG_categ, aes(x = Diagnosis, y = DMPmethylation, fill = Diagnosis)) +
    geom_violin(scale = "width", alpha = 0.3) + 
    geom_boxplot(width = 0.1, fill = "white") +
    geom_point(data = df_summary, aes(y = Mean, x = Diagnosis), color = "#14747b", size = 1) +
    th + th_transparent +
    ggtitle(paste0(cgID, "\n", Gene)) +
    labs(x = NULL, y = "CpG DNAm") + 
    scale_fill_manual(values = c("CTRL" = "#084081", "OCD" = "#891a1a"))+
    theme(legend.position = "none") +
    theme(plot.title = element_text(hjust = 0.5))
  Categ_plot
  ggsave(paste0("output_meta/Figures/DMPs/DMP_CaseCtrl_", cgID, "_", Gene2, "_CATEG.pdf"), Categ_plot, units = "cm", width = 7, height = 10.5, dpi = 400)
  ggsave(paste0("output_meta/Figures/DMPs/DMP_CaseCtrl_", cgID, "_", Gene2, "_CATEG.svg"), Categ_plot, units = "cm", width = 7, height = 10.5, dpi = 400)
  save(Categ_plot, df_summary, cgID, Gene, file = paste0("output_meta/Figures/DMPs/raw/DMP_CaseCtrl_", cgID, "_", Gene2, "_CATEG.RData"))
  ...
  layout <- (Categ_plot + Dens_plot) +
    plot_layout(design = design) +
    plot_annotation(title = paste0(cgID, " - ", Gene),
                    theme = theme(plot.title = element_text(size = 20, face = "bold", hjust = 0.5)))
  
  
  ggsave(paste0("output_meta/Figures/DMPs/combined/DMP_CaseCtrl_", cgID, "_", Gene2, "_COMB.svg"), layout, units = "cm", width = 19, height = 13, dpi = 400)
  ggsave(paste0("output_meta/Figures/DMPs/combined/DMP_CaseCtrl_", cgID, "_", Gene2, "_COMB.pdf"), layout, units = "cm", width = 19, height = 13, dpi = 400)       
  
  rm(layout)
  
}

signif_DMPs_BH$avg_diff_OCD_Ctrl <- signif_DMPs_BH$avg_OCD - signif_DMPs_BH$avg_CTRL
signif_DMPs_BH$median_diff_OCD_Ctrl <- signif_DMPs_BH$median_OCD - signif_DMPs_BH$median_CTRL


# EXPORT DMR TABLE WITH ADDED ORs
write_xlsx(signif_DMPs_BH, path = "output_meta/Tables/DMPs_CaseCtrl_limma_signif_BACON_BH_extended.xlsx")
save(signif_DMPs_BH, file = "output_meta/RData/DMPs_CaseCtrl_limma_signif_BACON_BH_extended.RData")
...
load(beta_path)
beta <- beta[row.names(beta) != "cg05575921", order(colnames(beta))] # remove cpg that is used to adjust for smoking
...
Mvalues_unmed <- Mvalues[,samples_unmed$Basename]
...
Mvalues_wobatch_unmed <- removeBatchEffect(Mvalues_unmed, covariates = covar_matrix, batch = as.factor(samples_unmed$Sex))
rm(covar_matrix)
...
for (j in c(1:nrow(DMRs_DMPs))){
  ...
  lm_result <- summary(lm(avgDNAm ~ CaseCtrl + Age + cell_PC1 + cell_PC2 + smoking + 
                            Ctrl_PC1 + Ctrl_PC2 + Ctrl_PC3 + Ctrl_PC5 +
...
save(unmedicated_result, file = "output_meta/RData/Sensitivity_unmedicated.RData")
write_xlsx(unmedicated_result, path = "output_meta/Tables/Sensitivity_unmedicated.xlsx")
...
Mvalues_noComorb <- Mvalues[, colnames(Mvalues) %in% samples_noComorb$Basename]
...
save(noComorb_result, file = "output_meta/RData/Sensitivity_noComorb.RData")
write_xlsx(noComorb_result, path = "output_meta/Tables/Sensitivity_noComorb.xlsx")
...
nrow(samples_filt) #604
length(unique(samples_filt$Resp_nr)) #604


# Genotyping SNPs
Matt_translation_filtered <- samplesheet_comb[which(samplesheet_comb$Resp_nr %in% samples_filt$Resp_nr), ]
write.table(Matt_translation_filtered[, c("Resp_nr", "Resp_nr")], file = "S:/Project/WP-genetics/05_Kira_mQTL/5_Clumping/meta_filtered_sampleIDs.txt",
            row.names = FALSE, col.names = FALSE, quote = FALSE)

stop("PERFORM PCA IN PLINK. This is not an error.")

snpsprobe_PCAscores <- read.table("S:/Project/WP-genetics/05_Kira_mQTL/5_PCA_results/CaseCtrl/PCA_result_meta.eigenvec", header = FALSE)
...
Mvalues_wobatch_genotypes <- removeBatchEffect(Mvalues_filt, covariates = covar_matrix, batch = sex)
rm(covar_matrix)
...
write_xlsx(mQTL_results_comb_sign, path = "output_meta/Tables/mQTL_sign.xlsx")
save(mQTL_results_comb_sign, file = "output_meta/RData/mQTL_sign.RData")

write_xlsx(mQTL_results_comb, path = "output_meta/Tables/mQTL.xlsx")
save(mQTL_results_comb, file = "output_meta/RData/mQTL.RData")
...
write.table(summary_stats_plink, "S:/Project/WP-genetics/05_Kira_mQTL/5_Clumping/CaseCtrl/association_results_meta.assoc",
            quote = FALSE, row.names = FALSE, sep = "\t")

stop("Clumping - not an error")
# 47 clumps formed from 556 top variants

clumped_snps <- read.table("S:/Project/WP-genetics/05_Kira_mQTL/5_Clumping/CaseCtrl/clumped_snps_final_meta.clumped", 
...
write_xlsx(mQTL_results_comb_clumped, path = "output_meta/Tables/mQTL_sign_clumped.xlsx")
save(mQTL_results_comb_clumped, file = "output_meta/RData/mQTL_sign_clumped.RData")

table(mQTL_results_comb_clumped$DMR)
# ARHGEF17     C13orf46;RASA3         cg01188427         cg02456934         cg02462015         cg02776498         cg03863908 
# 1                  3                  1                  1                  1                  1                  2 
# cg09816471         cg10000558         cg11415512         cg15294391         cg17428739         cg17725001         cg18693051 
# 1                  3                  5                  3                  1                  2                  3 
# cg19193420         cg19627034         cg20864214         cg22082184         cg27492584              CLDN9              DCHS1 
# 2                  3                  1                  1                  1                  3                  1 
# FAM120B;DLL1              KIFC3          LINC00511               LY6E              RBM47 ZNF833P;HNRNPA1P10 
# 2                  1                  1                  1                  2                  3
...
for (j in c(1:nrow(mQTL_results_comb_clumped))){
  ...
  if (nr_genotypes == 2){
    ...
    Diagnosis_SNP_count_plot <- ggplot(df_percentage, aes(x = SNP1, y = Percentage, fill = Diagnosis)) + 
      ...
      theme(legend.key = element_blank(),
            ...
            legend.title = element_blank(),
            plot.title = element_text(hjust = 0.5)) +
      scale_y_continuous(expand = c(0,0), limits = c(0, (max(df_percentage$Percentage) + 30))) +
  ...
  #save
  ggsave(paste0("output_meta/Figures/mQTL/DMR_CaseCtrl_", Region_Name2, "_", snp, "_DNAm_SNP.svg"), DNAm_SNP_plot, units = "cm", width = 6.5, height = 11, dpi = 400)
  ggsave(paste0("output_meta/Figures/mQTL/DMR_CaseCtrl_", Region_Name2, "_", snp, "_DNAm_SNP.pdf"), DNAm_SNP_plot, units = "cm", width = 6.5, height = 11, dpi = 400)       
  save(DNAm_SNP_plot, diff, file = paste0("output_meta/Figures/mQTL/DMR_CaseCtrl_", Region_Name2, "_", snp, "_DNAm_SNP.RData"))
  ggsave(paste0("output_meta/Figures/mQTL/DMR_CaseCtrl_", Region_Name2, "_", snp, ".svg"), Diagnosis_SNP_plot, units = "cm", width = 6.5, height = 11, dpi = 400)
  ggsave(paste0("output_meta/Figures/mQTL/DMR_CaseCtrl_", Region_Name2, "_", snp, ".pdf"), Diagnosis_SNP_plot, units = "cm", width = 6.5, height = 11, dpi = 400)       
  save(Diagnosis_SNP_plot, df_percentage, file = paste0("output_meta/Figures/mQTL/DMR_CaseCtrl_", Region_Name2, "_", snp, ".RData"))
  ggsave(paste0("output_meta/Figures/mQTL/DMR_CaseCtrl_", Region_Name2, "_", snp, "_count.svg"), Diagnosis_SNP_count_plot, units = "cm", width = 6.5, height = 11, dpi = 400)
  ggsave(paste0("output_meta/Figures/mQTL/DMR_CaseCtrl_", Region_Name2, "_", snp, "_count.pdf"), Diagnosis_SNP_count_plot, units = "cm", width = 6.5, height = 11, dpi = 400)       
  save(Diagnosis_SNP_count_plot, df_percentage, file = paste0("output_meta/Figures/mQTL/DMR_CaseCtrl_", Region_Name2, "_", snp, "_count.RData"))
  ...
  ggsave(paste0("output_meta/Figures/mQTL/combined/DMR_CaseCtrl_", Region_Name2, "_", snp, "_COMB.svg"), layout, units = "cm", width = 19, height = 12, dpi = 400)
  ggsave(paste0("output_meta/Figures/mQTL/combined/DMR_CaseCtrl_", Region_Name2, "_", snp, "_COMB.pdf"), layout, units = "cm", width = 19, height = 12, dpi = 400)       
...
write.table(summary_stats_plink, "S:/Project/WP-genetics/05_Kira_mQTL/5_Clumping/CaseCtrl/association_results_DNAm_meta.assoc",
            quote = FALSE, row.names = FALSE, sep = "\t")

stop("Clumping - not an error")
# 51 clumps formed from 560 top variants

clumped_snps_DNAm <- read.table("S:/Project/WP-genetics/05_Kira_mQTL/5_Clumping/CaseCtrl/clumped_snps_final_DNAm_meta.clumped", 
                                header = TRUE, stringsAsFactors = FALSE)

mQTL_results_comb_clumped_DNAm <- mQTL_results_comb_sign[which(mQTL_results_comb_sign$SNP %in% clumped_snps_DNAm$SNP), ]

write_xlsx(mQTL_results_comb_clumped_DNAm, path = "output_meta/Tables/mQTL_sign_clumped_DNAm.xlsx")
save(mQTL_results_comb_clumped_DNAm, file = "output_meta/RData/mQTL_sign_clumped_DNAm.RData")
...
for (j in c(1:nrow(mQTL_results_comb_clumped_DNAm))){
  ...
  #save
  ggsave(paste0("output_meta/Figures/mQTL_DNAm/DMR_CaseCtrl_", Region_Name2, "_", snp, "_DNAm_SNP.svg"), DNAm_SNP_plot, units = "cm", width = 6.5, height = 11, dpi = 400)
  ggsave(paste0("output_meta/Figures/mQTL_DNAm/DMR_CaseCtrl_", Region_Name2, "_", snp, "_DNAm_SNP.pdf"), DNAm_SNP_plot, units = "cm", width = 6.5, height = 11, dpi = 400)       
  save(DNAm_SNP_plot, diff, file = paste0("output_meta/Figures/mQTL_DNAm/DMR_CaseCtrl_", Region_Name2, "_", snp, "_DNAm_SNP.RData"))
}
...
LD_SNPs <- as.data.frame(read_xlsx("output_meta/Tables/LD_CaseCtrl_Meta_rs78454796.xlsx"))
mQTL_LD <- mQTL_results_comb[which(mQTL_results_comb$SNP %in% LD_SNPs$rsID), ]
write_xlsx(mQTL_LD, "output_F/Tables/mQTL_LD.xlsx")

LD_SNPs_own <- as.data.frame(read_xlsx("S:/Project/WP-genetics/05_Kira_mQTL/6_LD_Structure/CaseCtrl_Meta_LD.xlsx"))
