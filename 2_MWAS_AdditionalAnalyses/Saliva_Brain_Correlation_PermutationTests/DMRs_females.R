...
# Import
load("Users/kirahoeffler/OneDrive - University of Bergen/PhD/24_Case_Control_project/Comparison_GWAS/Non_DMR_regions.RData")
our_DMRs <- as.data.frame(read_xlsx("Users/kirahoeffler/OneDrive - University of Bergen/PhD/24_Case_Control_project/Export_SAFE/output_F/Tables/DMRs_CaseCtrl_15CtrlPCs_2AncPCs_extended.xlsx"))
...
genome <- getGenomeAndMask(data.frame(CHR = paste0("chr", c(1:22, "X", "Y")), 
                                      chr_length = c(249250621, 243199373, 198022430, 191154276, 180915260, 171115067,
                                                     ...
                                                     59128983, 63025520, 48129895, 51304566, 155270560, 0)))
...
for (j in 1:nrow(our_DMRs)){
  ...
  p_val_perm <- (sum(rand_median >= real_median)) / (length(rand_median))
...
write_xlsx(our_DMRs, path = "Users/kirahoeffler/OneDrive - University of Bergen/PhD/24_Case_Control_project/Saliva_brain/Permutation_test/DMRs_females.xlsx")
