...
#' Adapt dmrff results
#'
#' adapts output of dmrff function
#' IMPORTANT: THIS FUNCTION WAS WRITTEN FOR EPICv2 - might need some adaptation for other arrays
#' 
#' @param dmrff_result direct output of dmrff() function, with columns p.adjust, n, chr, start, end
#' @param DMPs_anno merged DMP results (summary statistics) with annotation, with columns P.Value, UCSC_RefGene_Name, CHR, MAPINFO, cgID
#' @param annotation annotation as downloaded from Illumina
#' @param nr_signif_CpGs DMR should have at least X nominally significant CpGs (insert a number for X, e.g. 3)
#'
#' @return adapted dmrff df
#'  
adapt_dmrff <- function(dmrff_result, DMPs_anno, annotation, nr_signif_CpGs){
  ...
  print(paste0("significant DMRs before filtering step: ",nrow(dmrff_sign)))
  ...
  print(paste0("significant DMRs after filtering: ", nrow(dmrff_sign)))
...
#'
#' @param dmrff_sign output of adapt_dmrff
#' @param annotation  annotation as downloaded from Illumina (EPICv2)
#'
#' @return adapted dmrff_sign
#'
add_location <- function(dmrff_sign, annotation){
  
  #annotate DMP_overlap
  dmrff_sign$Gene_relation <- NA
  dmrff_sign$CpG_island_relation <- NA
  ...
  for (i in c(1:nrow(dmrff_sign))){
    ...
    dmrff_sign$Gene_relation[i] <- Gene_relation_String
    ...
    dmrff_sign$CpG_island_relation[i] <- GeneIsland_relation_String
...
#' @param signif_DMPs merged DMP results (summary statistics) with annotation, with column cgID,
#' filtered on DMPs that reach significance after multiple testing (not a problem if this is an empty df)
#' @param all_DMR_cpgs output from extract_CpGs_from_DMRs() function
#'
#' @return Manhatten plot object
#' 
Manhattan <- function(DMPs_anno, sex, signif_DMPs, all_DMR_cpgs){
  ...
  Manhattan_plot <- ggplot() +
    ...
    geom_hline(yintercept = (-log10(min_p)), color = "grey", linetype = "dashed", alpha = 0.5) +
